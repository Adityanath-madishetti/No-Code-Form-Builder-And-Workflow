...
import type { AnyFormComponent } from '../utils/componentRegistry';
...
import { DEFAULT_FORM_THEME } from '../config/formTheme';
// import {
  ...
  immer((set, get) => ({
    ...
    updatePageTerminal: (pageId: PageID, isTerminal: boolean) =>
      set((state) => {
        if (!state.form || state.form.pages.length === 0) return;
        const isLastPage =
          state.form.pages[state.form.pages.length - 1] === pageId;
