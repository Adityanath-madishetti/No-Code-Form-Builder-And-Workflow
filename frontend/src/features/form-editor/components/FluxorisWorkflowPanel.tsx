...
export function FluxorisWorkflowPanel() {
  ...
  const [, setExchangeError] = useState('');
  ...
  const fetchRuns = useCallback(
    async (workflowId: string) => {
      ...
      try {
        ...
        if (res.status === 404) {
          setIsServiceDown(true);
          throw new Error('Fluxoris service returned 404');
        }
          ...
          <div className="space-y-4">
            <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed bg-muted/30 p-6 text-center">
              {!fluxorisToken ? (
                ...
                <>
                  <AlertCircle className="mb-3 h-10 w-10 text-destructive opacity-50" />
                  <h3 className="text-sm font-medium">Fluxoris Service Down</h3>
                  <p className="mt-1 mb-4 text-[10px] text-muted-foreground">
                    The workflow service is currently unreachable (404). Please
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (connectedWorkflow?.workflow_id) {
                        ...
                        // Re-trigger connection fetch by cycling a dependency if needed,
                        // or just rely on the effect running again if we add a retry state.
                        // For now, let's just force a refresh of the token which will trigger effects.
                        handleExchangeToken();
                      }
                    }}
                  >
