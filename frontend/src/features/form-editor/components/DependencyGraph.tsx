...
import { useLogicStore, getDependencyEdges } from '@/features/form';
import { useFormStore } from '@/features/form/stores/form.store';
import {
  ACTION_TYPE_LABELS,
  ACTION_TYPE_COLORS,
} from '@/features/form/types/logic.types';
...
export function DependencyGraph() {
  ...
  const getLabel = (id: string): string => {
    if (components[id])
      return components[id].metadata.label || components[id].id;
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-1.5">
        ...
        {edges.map((edge, i) => {
          const ruleName =
            rules.find((r) => r.ruleId === edge.ruleId)?.name || 'Unknown';
          return (
            <div
              key={i}
            ...
            >
              {/* Source */}
              <span className="min-w-0 truncate rounded bg-blue-500/10 px-1.5 py-0.5 font-mono text-[10px] text-blue-600">
              ...
              {/* Action */}
              <span
                className={`shrink-0 text-[10px] font-semibold ${ACTION_TYPE_COLORS[edge.actionType]}`}
              >
              ...
              {/* Target */}
              <span className="min-w-0 truncate rounded bg-amber-500/10 px-1.5 py-0.5 font-mono text-[10px] text-amber-600">
